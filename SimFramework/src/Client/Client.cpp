...
void separated_tests(const configs* const settings, secure_connection* conn) {
    ...
    if(settings->use_images) {
        ...
        if(settings->tcbir_dataset_rec)
            files = list_img_files_rec(!settings->tcbir_nr_docs? -1 : settings->tcbir_nr_docs, settings->tcbir_dataset_dir);
        else
            files = list_img_files(!settings->tcbir_nr_docs? -1 : settings->tcbir_nr_docs, settings->tcbir_dataset_dir);
        ...
        if(!strcmp(settings->tcbir_train_technique, "client_kmeans"))
            ...
            tcbir_train_iee_kmeans(conn, settings->tcbir_train_mode, desc, files);
        ...
        if(!strcmp(settings->tcbir_add_mode, "normal")) {
            tcbir_add_files(conn, desc, files);
        ...
        if(settings->tcbir_nr_queries > 0) {
            // flickr
            search_flickr(conn, desc, list_img_files(-1, settings->tcbir_dataset_dir), settings->tcbir_nr_queries);
        } else {
            // search test (inria)
