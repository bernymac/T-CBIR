...
void tcbir_setup(secure_connection* conn, size_t desc_len, unsigned tcbir_nr_clusters, const char* train_technique) {
    ...
    init(&in, &in_len, tcbir_nr_clusters, desc_len, train_technique);
...
void tcbir_train_client_kmeans(secure_connection* conn, unsigned tcbir_nr_clusters, char* tcbir_train_mode, char* tcbir_centroids_file, const char* tcbir_dataset_dir, feature_extractor desc) {
    size_t in_len = sizeof(uint8_t) + tcbir_nr_clusters * desc.get_desc_len() * sizeof(float);
    uint8_t* in = (uint8_t*)malloc(sizeof(uint8_t) + tcbir_nr_clusters * desc.get_desc_len() * sizeof(float));
    in[0] = OP_IEE_SET_CODEBOOK_CLIENT_KMEANS;

    float* centroids;
    if(!strcmp(tcbir_train_mode, "train")) {
        centroids = client_train(tcbir_dataset_dir, tcbir_nr_clusters, desc.get_desc_len(), desc);

        FILE* file = fopen(tcbir_centroids_file, "wb");
        if (!file) {
            printf("Could not write centroids file\n");
            exit(1);
        }

        fwrite(centroids, tcbir_nr_clusters, desc.get_desc_len() * sizeof(float), file);
        fclose(file);
    } else if(!strcmp(tcbir_train_mode, "load")) {
        centroids = (float*)malloc(tcbir_nr_clusters * desc.get_desc_len() * sizeof(float));
        FILE* file = fopen(tcbir_centroids_file, "rb");
        if (!file) {
            printf("Could not read centroids file\n");
            exit(1);
        }

        fread(centroids, tcbir_nr_clusters, desc.get_desc_len() * sizeof(float), file);
        fclose(file);
    } else {
    ...
    memcpy(in + 1, centroids, tcbir_nr_clusters * desc.get_desc_len() * sizeof(float));
    iee_comm(conn, in, in_len);
    free(in);
    free(centroids);
}
...
void tcbir_train_client_lsh(secure_connection* conn, char* tcbir_train_mode, size_t desc_len, unsigned tcbir_nr_clusters) {
    size_t in_len = sizeof(uint8_t) + tcbir_nr_clusters * desc_len * sizeof(float);
    uint8_t in[in_len];

    vector<float*> gaussians(tcbir_nr_clusters);
    std::default_random_engine generator(time(0));
    std::normal_distribution<float> distribution(0.0, 25.0);

    for (unsigned i = 0; i < tcbir_nr_clusters; ++i) {
    ...
    for (unsigned k = 0; k < tcbir_nr_clusters; ++k) {
...
void tcbir_add_files(secure_connection* conn, feature_extractor desc, const vector<string> files) {
    ...
    printf("-- T-CBIR TOTAL add: %lf ms (%lu imgs) --\n", total_client + total_iee, files.size());
    printf("-- T-CBIR add client: %lf ms (of which %lf ms extract, %lf ms read) --\n", total_client, get_feature_time_add(), get_read_time_add());
    printf("-- T-CBIR add iee w/ net: %lf ms --\n", total_iee);
    printf("-- T-CBIR add networking: %lf ms --\n", network_time);
}
