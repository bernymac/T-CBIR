...
void* process_client(void* args) {
    ...
    while (1) {
        ...
        switch (op) {
            case OP_UEE_INIT: {
#if STORAGE == STORAGE_CASSANDRA
                execute_query(session, "CREATE KEYSPACE IF NOT EXISTS tcbir WITH REPLICATION = {'class' : 'SimpleStrategy', 'replication_factor' : 1};");
                execute_query(session, "DROP TABLE IF EXISTS tcbir.pairs;");
                execute_query(session, "CREATE TABLE tcbir.pairs (key blob, value blob, PRIMARY KEY (key));");
#elif STORAGE == STORAGE_REDIS
